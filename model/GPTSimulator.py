...
class GPTSimulator(nn.Module):
    def __init__(self, train_example_nums, test_example_nums, hyper_parameter=0, **kwargs):
        super(GPTSimulator, self).__init__()
        # 编码器和tokenzier
    ...
    def forward(
        ...
        **kwargs
        ...
        if self_influence:
            x_src = self.embed(orders) # (1, 1, hid_dim)
            x_tgt = self.embed(test_sample_ids) # (1, hid_dim)
            if not self.concate:
                # 计算a
                x_src_a = self.mlp_a(x_src) # (1, 1, hid_dim)
                x_tgt_a = self.mlp_a(x_tgt) # (1, hid_dim)
                a = torch.mul(x_src_a, x_tgt_a.unsqueeze(1)).sum(dim=-1) # (1, 1, hid_dim)
                ...
                x = self.fc(x)
                x = self.relu(x)
                a = self.fca(x).squeeze(-1) # (1, 1, hid_dim)
                b = self.fcb(x).squeeze(-1) # (1, 1, hid_dim)
            
            return {
                'self_influence_a': a.item(),
                'self_influence_b': b.item(),
            }
        else:
            ...
            if not self.concate:
                ...
                a = torch.mul(x_src_a, x_tgt_a.unsqueeze(1)).sum(dim=-1) # (bs, ft_bs)

                # 计算b
                x_src_b = self.mlp_b(x_src)
                x_tgt_b = self.mlp_b(x_tgt)
                b = torch.mul(x_src_b, x_tgt_b.unsqueeze(1)).sum(dim=-1) # (bs, ft_bs)
            else:
                x = torch.cat([x_src, x_tgt.unsqueeze(1).repeat(1, ft_bs, 1)], dim=-1) # (bs, ft_bs+1, hid_dim)])
                x = self.fc(x)
                x = self.relu(x)
    ...
    with torch.no_grad():
        def _get_sample_embeds(self, texts, device):
            ...
            '''
            tokenized = self.tokenizer(texts, return_tensors='pt', padding=True, truncation=True, max_length=self.max_input_length)
            ...
            embeds = output.last_hidden_state[:, -1]
            return embeds
